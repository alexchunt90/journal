...
// Tracked files plus untracked ones git would not ignore, so a fresh scaffold
// with nothing committed yet is still checked.
const files = execFileSync('git', ['ls-files', '--cached', '--others', '--exclude-standard', '--', '*.js'], { cwd: ROOT, encoding: 'utf8' })
  .split('\n').filter(Boolean);
